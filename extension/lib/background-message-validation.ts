...
function isString(value: unknown, maximumLength = MAX_ID_LENGTH): value is string {
  return typeof value === 'string' && value.length > 0 && value.length <= maximumLength;
}

function isOptionalString(value: unknown, maximumLength = MAX_ID_LENGTH): value is string | undefined {
...
/** Rejects content scripts and child frames from lifecycle/destructive UI controls. */
...
export function isExtensionPageOnlyMessage(message: BackgroundMessage): boolean {
  return message.type === 'START_RECORDING' ||
    ...
    RECORDING_CONTROLS.has(message.type);
}

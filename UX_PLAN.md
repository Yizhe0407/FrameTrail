...
> **狀態（2026-07-24）：本節色彩／字體 token 已被取代。** 專案改採 Claude Design 匯入的
> 「FrameTrail Prototype.dc.html」視覺語言（黑／白 primary、藍與薰衣草 accent、16px 卡片圓角、
> pill 造型控制項），套用於 `extension/assets/tailwind.css` 及所有畫面。以下 §12.1–§12.4
> 保留作為歷史紀錄與驗證方法論參考（對比驗證流程、間距／可及性原則多數仍適用），但表列的
> 實際色票、字體堆疊與圓角數值已不是目前實作。§13 的無障礙驗收基線與量測方法不受影響。
